...
### Added
- Review campaigns: creating a campaign now asks which published agent version it runs on (defaulting to the latest), the campaign editor shows and can re-pin that version while the campaign is still a draft, and tester sessions run on the pinned version instead of the newest one; existing campaigns are re-pinned to their agent's latest published version
