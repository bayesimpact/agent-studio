...
import { getAgentExtractionRun } from "../agent-extraction-runs.thunks"

export function ExtractionRunItem({ run }: { run: AgentExtractionRunSummary }) {
  ...
  const handleGetRunResult = async (runId: string): Promise<Record<string, unknown> | null> => {
    ...
    setIsLoading(true)
    try {
      const runDetails = await dispatch(getAgentExtractionRun({ runId })).unwrap()
...
function JsonViewer({
  ...
  const { t } = useTranslation("agentExtractionRun", { keyPrefix: "result.view" })
...
function JsonDownloader({
  ...
  const { t } = useTranslation("agentExtractionRun", { keyPrefix: "result.download" })
