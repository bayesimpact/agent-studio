import {
  type UpdateAgentSettingsResourcesDto,
  updateAgentSettingsResourcesSchema,
...
import { updateAgentSettingsResources } from "@/common/features/agents/agent-settings/agent-settings.thunks"
...
export function ResourceLibrariesTab({ agentSettings, onDirtyChange }: AgentTabFormProps) {
  ...
  const form = useForm<UpdateAgentSettingsResourcesDto>({
    resolver: zodResolver(updateAgentSettingsResourcesSchema),
    defaultValues: { resourceLibraryIds: agentSettings.resourceLibraryIds },
  })
  useReportDirty(form.formState.isDirty, onDirtyChange)

  const handleSubmit = form.handleSubmit(async (values) => {
    await dispatch(
      updateAgentSettingsResources({ agentId: agentSettings.agentId, fields: values }),
    ).unwrap()
    form.reset(values)
  })
