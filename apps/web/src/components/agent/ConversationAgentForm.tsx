...
import { type AgentFormData, buildAgentSchema, getDefaultFormValues } from "./agent-form.shared"
...
export function ConversationAgentForm({
  ...
  const agentSchema = buildAgentSchema((key) => t(`agent:props.${key}`))
  ...
  const {
    ...
    defaultValues: (editableAgent ?? getDefaultFormValues("conversation")) as FormValues,
  })
