...
export function ExtractionAgentForm({
  defaultValues = getDefaultFormValues("extraction"),
  isLoading,
  error,
  onSubmit,
  submitLabelIdle,
  ...
  const agentSchema = buildAgentSchema(t).extend({
    outputJsonSchemaText: z
      .string()
      .min(1, t("validation.outputJsonSchemaRequired"))
      .refine(isValidJsonObject, t("validation.outputJsonSchemaInvalid")),
  })
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)}>
      <CardContent className="space-y-4 mt-2 px-0">
        ...
        <div className="space-y-2">
          <Label htmlFor="defaultPrompt">{t("labelInstructionsPrompt")}</Label>
          <Textarea
            id="defaultPrompt"
            placeholder={t("placeholderInstructionsPrompt")}
            rows={8}
            className="min-h-40"
        ...
        <div className="space-y-2">
          <Label htmlFor="outputJsonSchemaText">{t("labelOutputJsonSchema")}</Label>
          <Textarea
            id="outputJsonSchemaText"
            placeholder={t("placeholderOutputJsonSchema")}
            rows={10}
