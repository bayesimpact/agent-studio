...
import { JobSearchResponse } from './types/job-offer.types.js';
import { SimplifiedJobOffer } from './models/simplified-job-offer.model.js';
...
import { FranceTravailBaseService } from './francetravail-base.service.js';
  ...
  implements AIServiceProvider
{
  ...
  getFunctionDeclaration(): FunctionDeclaration {
    return {
      name: 'jobs_search',
      description:
        'Search for a job, you can automatically propose job titles based on the context you have',
      parameters: {
        type: Type.OBJECT,
        properties: {
          jobTitles: {
            type: Type.ARRAY,
            description:
              'possible jobs title in french, e.g. boucher or chauffeur de bus',
            items: {
              type: Type.STRING,
            },
          },
...
`;
  ...
  async executeFunction(
    ...
    if (functionCall.args) {
      ...
      console.error('Function calling with params:', jobTitles, departmentsCode);

      const jobOffers = await this.searchJobOffers({
        jobTitles,
        departmentsCode,
      });
      console.error('Job offers length: ', jobOffers.length);
      return { jobOffers };
    }
  ...
  async searchJobOffers({
    ...
    console.error(`Calling job search with ${codeROME} ${departement}`);
    ...
    if (!data.resultats || data.resultats.length === 0) {
      console.error('No job offers found');
      return [];
    }
