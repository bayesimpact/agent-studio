...
import { EventSearchResponse } from './types/event.types.js';
import { SimplifiedEvent } from './models/simplified-event.model.js';
...
`;
  ...
  async executeFunction(
    ...
    if (functionCall.args) {
      ...
      const departmentCode = functionCall.args['departmentCode'] as string;

      console.error(
        'Function calling events with params:',
        jobTitles,
        departmentCode,
        endDate,
      );
      ...
      console.error('Events found: ', events.length);
      return { events };
    }
    return { events: [] };
  }
