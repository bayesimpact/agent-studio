...
} from './care-plan-builder.abstract.js';
import {
  GoogleGenAI,
  ToolListUnion,
  FunctionCall,
...
import {
  ...
  buildUserPrompt,
} from './prompts/care-plan-builder.prompt.js';
...
export class AICarePlanBuilderService {
    ...
    for (const functionCall of functionCalls) {
        if (functionCall.args) {
          ...
          if (functionCall.name === 'workshops_search') {
            ...
            result =
              await this.franceTravailEventsService.executeFunction(
                functionCall,
              );
            source = 'francetravail';
            console.error(
              `✅ Event search returned ${result.events?.length || 0} results`,
            );
          } else if (functionCall.name === 'services_search') {
            result =
              await this.dataInclusionService.executeFunction(functionCall);
            source = 'datainclusion';
            console.error(
              `✅ Services search returned ${result.services?.length || 0} results`,
            );
          }

          functionResults.push({
            name: functionCall.name || 'func',
            result,
          });
