...
import { SimplifiedWorkshop } from './models/simplified-workshop.model.js';
...
export class NotionWorkshopService implements AIServiceProvider {
  ...
  private readonly notionDatabaseId =
    process.env.NOTION_DATABASE_ID || 'e3b5ba04a6b94e6a845c604ae46bcee6';
...
`;
  ...
  async executeFunction(
    ...
    if (functionCall.args) {
      ...
      console.error('Workshops found: ', workshops.length);
      return { workshops };
    }
    return { workshops: [] };
  }
  ...
  async searchWorkshops(): Promise<SimplifiedWorkshop[]> {
    ...
    try {
      console.error(`Calling workshops search (returning all items)`);
      ...
      console.error(`Found ${data.results.length} workshops in Notion database`);
