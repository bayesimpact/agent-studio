...
describe("Auth0UserInfoService", () => {
          ...
          useValue: {
            get: jest.fn((key: string) => {
              if (key === "AUTH0_ISSUER_URL") {
                return "https://bayes-impact.eu.auth0.com/"
              }
              return undefined
            }),
          },
  ...
  describe("getUserInfo", () => {
    it("should fetch user info from Auth0 UserInfo endpoint", async () => {
      ...
      // Assert
      expect(global.fetch).toHaveBeenCalledWith("https://bayes-impact.eu.auth0.com/userinfo", {
        method: "GET",
        headers: {
    ...
    it("should handle Auth0 issuer URL with trailing slash", async () => {
      ...
      const configServiceWithSlash = {
        get: jest.fn((key: string) => {
          if (key === "AUTH0_ISSUER_URL") {
            return "https://bayes-impact.eu.auth0.com/" // With trailing slash
          }
          return undefined
        }),
      }
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "https://bayes-impact.eu.auth0.com/userinfo",
        expect.any(Object),
      )
    })
