...
import type { ServiceSearchResponse } from "./types/service-search.types"
...
export class DataInclusionService implements AIServiceProvider {
  ...
  getFunctionDeclaration(): FunctionDeclaration {
    return {
      ...
      description: "Service search allows finding services based on thematic categories.",
      parameters: {
        type: Type.OBJECT,
        properties: {
          thematiques: {
            type: Type.ARRAY,
            description: "Thematic categories, multiple themes can be specified",
            items: {
              type: Type.STRING,
              enum: thematiques,
            },
...
`
  ...
  async executeFunction(functionCall: FunctionCall): Promise<{ services: SimplifiedService[] }> {
  ...
  private validateThematiques(thematiquesInput: string[]): string[] {
    ...
    if (validThematiques.length === 0 && thematiquesInput.length > 0) {
      console.warn("⚠️  All provided thematiques were invalid. No services will be searched.")
    }
  ...
  async searchServices({
    ...
    const { data } = await axios.get<ServiceSearchResponse>(`${this.baseUrl}/search/services`, {
      params: {
