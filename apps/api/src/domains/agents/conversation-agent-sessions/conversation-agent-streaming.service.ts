...
export class AgentStreamingService extends ServiceWithLLM {
  ...
  async *streamAgentResponse({
    ...
    const { session: updatedSession, assistantMessageId } =
      await this.conversationAgentSessionsService.prepareForStreaming({
        connectScope,
        sessionId,
        userContent,
        documentId,
    ...
    try {
      ...
      await this.conversationAgentSessionsService.finalizeStreaming(
        updatedSession.id,
        assistantMessageId,
        fullContent,
      )
      ...
      await this.conversationAgentSessionsService.markStreamingError(
        updatedSession.id,
        assistantMessageId,
        errorMessage,
      )
