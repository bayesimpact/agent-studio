...
import { AgentMembershipPolicy } from "./agent-membership.policy"

@Injectable()
export class AgentMembershipsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const policy = new AgentMembershipPolicy(
      requestToProjectPolicyContext(request),
      "agentMembership" in request ? request.agentMembership : undefined,
    )
