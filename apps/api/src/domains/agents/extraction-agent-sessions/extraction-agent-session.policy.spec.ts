...
import { ExtractionAgentSessionPolicy } from "./extraction-agent-sessions.policy"

type RunState = "sameOrganization" | "differentOrganization" | "noRun"

describe("ExtractionAgentSessionPolicy", () => {
  ...
  const buildRun = (runState: RunState): ExtractionAgentSession | undefined => {
    if (runState === "sameOrganization") {
      const project = buildProject(organization)
      return { projectId: project.id } as ExtractionAgentSession
    }
  ...
  describe("canList", () => {
    describe.each<[MembershipRole, RunState, boolean]>([
      ...
      it(`should return ${expected}`, () => {
        const policy = new ExtractionAgentSessionPolicy(
          {
