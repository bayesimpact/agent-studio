...
const resolveEmbeddingModelName = (): string => {
  const modelName = toNonEmptyValue(process.env.DOCUMENT_EMBEDDING_MODEL)
  if (!modelName) {
    throw new InternalServerErrorException(
      "Missing embedding model configuration. Set DOCUMENT_EMBEDDING_MODEL.",
    )
  }
  return modelName
}
...
export class DocumentEmbeddingsProcessorService {
  ...
  async processDocument(payload: CreateDocumentEmbeddingsJobPayload): Promise<void> {
    ...
    try {
      ...
      const embeddingModelName = resolveEmbeddingModelName()
      this.logger.log(
        `Creating embeddings with Vertex model ${embeddingModelName} in project ${project}, location ${location}`,
      )
