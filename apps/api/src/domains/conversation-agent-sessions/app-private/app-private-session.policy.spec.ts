...
describe("AppPrivateSessionPolicy", () => {
  ...
  const buildPolicy = (role: MembershipRole, projectMembershipState: ProjectMembershipState) => {
    return new AppPrivateSessionPolicy({
  ...
  describe("canList", () => {
    it("should return true for owner", () => {
      const policy = buildPolicy("owner", "noProjectMembership")
      expect(policy.canList()).toBe(true)
    })

    it("should return true for admin", () => {
      const policy = buildPolicy("admin", "noProjectMembership")
      expect(policy.canList()).toBe(true)
    })

    it("should return true for member of the same project", () => {
      const policy = buildPolicy("member", "sameProject")
      expect(policy.canList()).toBe(true)
    })

    it("should return false for member without project membership", () => {
      const policy = buildPolicy("member", "noProjectMembership")
      expect(policy.canList()).toBe(false)
    })

    it("should return false for member of another project", () => {
      const policy = buildPolicy("member", "differentProject")
      expect(policy.canList()).toBe(false)
    })
  })
