...
import { EvaluationExtractionRunPolicy } from "./evaluation-extraction-run.policy"

@Injectable()
export class EvaluationExtractionRunGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const policy = new EvaluationExtractionRunPolicy(
      requestToProjectPolicyContext(request),
      request.evaluationExtractionRun,
    )
