...
async function main(): Promise<void> {
  ...
  try {
    console.log(
      `Cleaning ${workerCount} worker databases with base name "${baseDatabaseName}_w{n}"...`,
    )
    for (let workerIndex = 1; workerIndex <= workerCount; workerIndex += 1) {
      ...
      console.log(`Dropped database ${workerDatabaseName}`)
    }
  } finally {
...
main().catch((error) => {
  console.error("Failed to cleanup test worker databases:", error)
  process.exit(1)
})
