...
import { createSingleUser } from "@/users/user.factory"
import { ChatMessage } from "./chat-message.entity"
import { chatMessageFactory, createChitChatConversation } from "./chat-messages.factory"
...
describe("ChatSessionsService", () => {
  ...
  beforeAll(async () => {
    setup = await setupTransactionalTestDatabase(
      [ChatSession, ChatMessage, ChatBot, User, Organization, Project, UserMembership],
      [],
      [ChatSessionsModule],
    )
  ...
  beforeEach(async () => {
    ...
    const insertedEntities = await createOrganizationWithChatBot({
      userRepository,
    ...
    testChatBot = insertedEntities.chatBot
    testUser = insertedEntities.user
    testOrganization = insertedEntities.organization
    testProject = insertedEntities.project
  })
  ...
  describe("createPlaygroundSession", () => {
    ...
    it("should reuse existing session if TTL not expired", async () => {
      ...
      await createChitChatConversation(session1, { chatMessageRepository })
      ...
      // Messages preserved - check via repository
      const messages = await chatMessageRepository.find({ where: { sessionId: session1.id } })
    ...
    it("should reset messages and update TTL if session expired", async () => {
      ...
      // Messages reset - check via repository
      const messages = await chatMessageRepository.find({ where: { sessionId: session1.id } })
      expect(messages).toHaveLength(0) // Messages reset
  ...
  describe("createPlaygroundSessionForChatBot", () => {
    ...
    it("should throw ForbiddenException when user is not a member of the organization", async () => {
      const anotherUser = await createSingleUser(userRepository, {
        email: `another-user-${Date.now().toString()}@example.com`,
      })
      await expect(
        service.createPlaygroundSessionForChatBot(testChatBot.id, anotherUser.id),
  ...
  describe("listMessagesForSession", () => {
    ...
    it("should throw ForbiddenException when user is not a member of the organization", async () => {
      ...
      await expect(service.listMessagesForSession(session.id, anotherUser.id)).rejects.toThrow(
        ForbiddenException,
      )
    })
  })
