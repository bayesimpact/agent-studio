...
describe("getAllApp", () => {
  it("should return all app-private sessions for a chatbot and user", async () => {
    ...
    const playgroundSession = chatSessionFactory.transient({ chatBot, user, organization }).build({
      type: "playground",
    ...
    const { data: result } = await controller.getAllApp(mockRequest, chatBot.id)
