...
import type { JobSearchResponse } from "./types/job-offer.types"
...
`
  ...
  async executeFunction(functionCall: FunctionCall): Promise<{ jobOffers: SimplifiedJobOffer[] }> {
...
${jobOffers
  .map(
    (job, index) => `${index + 1}. **${job.title}** - ${job.company}
   - Lieu: ${job.location}
