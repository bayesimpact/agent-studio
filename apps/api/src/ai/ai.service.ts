...
export class AIService {
  ...
  private buildSystemPrompt(country: "fr" | "us"): string {
    ...
    const toolContexts = allProviders.map((provider) => provider.getPromptContext()).join("\n\n")
...
`
    ...
    for (const message of chatSession.messages) {
      if (message.sender === "tool") {
        ...
        if (toolResponseParts.length > 0) {
          contents.push({ role: "user", parts: toolResponseParts as Part[] })
          toolResponseParts = []
        }
  ...
  async *generateContentStream({
    ...
    try {
      ...
      let lastChunck: GenerateContentResponse | undefined
      ...
      generation.update({
        output: functionCalls.length > 0 ? { text: fullOutput, functionCalls } : fullOutput,
        usage: {
  ...
  async *generateChatStream({
    ...
    try {
      ...
      generation.end()
      console.info(`LLM call completed. Tokens: ${lastChunck?.usageMetadata?.totalTokenCount}`)
    } catch (error) {
      // Log error to Langfuse
      generation.update({
        level: "ERROR",
